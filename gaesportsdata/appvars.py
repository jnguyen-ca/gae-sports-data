...
def handle_request():
    ...
    logging.debug('Request : (%s), AppVar : (%s)' % (request_type, app_var_id))
    
    #TODO: other request types (e.g. delete entry)
    if request_type == 'add-edit-entry':
        ...
        if key_list or entry_key:
            if entry_key:
                key_list.append(entry_key)
                
            logging.debug('Setting dict entry: %s' % (str(key_list)))
        ...
        #TODO: allow creation of new appvars from scratch
#         else:
#             models.ApplicationVariable.set_app_var(app_var_id, request_value)
    else:
        flask.abort(400)
